...
(Bun, Electron, новый Node) собраны под новые ОС. На старых Intel Mac (High Sierra
10.13 и ранее) они не запускаются. Этот проект — агент, который работает там, где
всё остальное — нет.
...
Для **gigachat** (бесплатно для физлиц — freemium-квоты, см. тарифы в доках Sber):
1. Зарегистрируйтесь в [Studio](https://studio.dev.sberbank.ru), создайте проект **GigaChat API**.
2. **Настройки API → Получить ключ** → сохраните **Client ID** и **Client Secret**
   (показываются один раз).
3. Запросы ходят на `api.giga.chat` с корневыми сертификатами НУЦ Минцифры, которых
   нет в стандартных хранилищах, поэтому для этого провайдера добавьте флаг
   **`--insecure`** (отключает проверку сертификата — только в доверенной сети).
...
Бинарник для `darwin-x64` собирает **GitHub Actions** (нужен macOS-раннер), а на
...
4. На целевом Mac (Rust не нужен):
...
На самом старом Intel Mac такая сборка возможна, но медленная (компиляция
